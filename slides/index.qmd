...
- I create my `environment.yml` with just the packages I'm using directly, and let `conda` figure out what other packages are needed to make them work
...
- This is optional; if you leave it out it will use the default channels
- If you install anaconda, it will use the `defaults` channel that contains the Anaconda distribution
- If you installed from `conda-forge`, it will use the `conda-forge` channel that is maintained by the community
    - Typically more up to date and complete, and does not have the same licensing restrictions as Anaconda
...
- There are two options for using JupyterLab with a conda environment
    ...
    - Install `ipykernel` (which we already did) in your environment and create a Jupyter "kernel" you can access from another environment

## Using Jupyter with Conda environments: VSCode

- Open the `Air analysis.ipynb` notebook in Visual Studio Code and select the "Kernel" to be `air-analysis` (the environment we just created)

## Using Jupyter with Conda environments: JupyterLab

- Let's install `jupyterlab` into our conda environment
- "Activate" your conda environment: `conda activate air-analysis`
- Edit your `environment.yml` to include a dependency `jupyterlab`
...
error_data.iloc[0, "Passengers"] = -1
busiest_routes(error_data, Origin, Dest)
```
...
- It is good practice to import any functions that you use from other libraries (e.g. `pd.read_csv`)

## Using your data cleaning function

- Delete the data loading code in each of the analysis files
- Restart the kernel (Kernel -> Restart Kernel in JupyerLab or "Restart" button in VSCode) to remove existing data from memory
